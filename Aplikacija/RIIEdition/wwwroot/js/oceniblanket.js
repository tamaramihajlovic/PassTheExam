...
window.addEventListener("DOMContentLoaded",()=>{
    ...
    Array.from(document.getElementsByClassName("in1")).forEach((eli,ind)=>eli.addEventListener("keyup",(ev)=>{
        console.log(ev.target.value)
     if(ev.target.value=="")
     Array.from(document.getElementsByClassName("comment1"))[ind].disabled=true;
     else
     Array.from(document.getElementsByClassName("comment1"))[ind].disabled=false; 
    }))
  let nizElemenata=  Array.from(document.getElementsByClassName("admin")).map(x=>x.parentElement.querySelector("h6"));
  nizElemenata.forEach(el=>el.classList.add("by-author"));
  let nizElemenata1=  Array.from(document.getElementsByClassName("professor")).map(x=>x.parentElement.querySelector("h6"));
...
function otvoryReplyBox(ev){
    ...
    ev.target.parentElement.parentElement.parentElement.parentElement.querySelector(".odgovorKomentara").style.display="list-item";
